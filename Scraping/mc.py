#Automatic scraper for Metacritic (To be run first to scrape mc)
...
for mov in mycursor.fetchall():
	print('**********',mov[0])
	url="https://www.metacritic.com/movie/"+re.sub(r"[\'\,\:\;\.\(\)]",'',unidecode.unidecode(mov[0].replace(' ','-'))).lower()+'-'+str(mov[1])
